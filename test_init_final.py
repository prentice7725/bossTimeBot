...
#				raise Exception("[fixed_boss.ini] 파일에서 [kakaoOnOff]를 지워주세요.")
	...
	try:
		...
		basicSetting.append(inputData[14][14:16])  #basicSetting[4] : restarttime 時
		basicSetting.append(inputData[14][17:])    #basicSetting[5] : restarttime 分
		basicSetting.append(inputData[1][15:])     #basicSetting[6] : voiceチャンネル ID
		basicSetting.append(inputData[2][14:])     #basicSetting[7] : textチャンネル ID
		...
		raise Exception("[test_setting.ini] 設定エラーです")
	...
	regenembed.set_footer(text = 'R : わきなし ')
			...
			if timestring == bossTime[i]:
				if bossTimeString[i] != '99:99:99' or bossMungFlag[i] == True :
					if bossMungFlag[i] == True :
						if bossData[i][2] == '0' :
							information1 += ' - ' + bossData[i][0] + '(' + bossData[i][1] + '.' + bossData[i][5] + ') : ' + tmp_bossTime[i].strftime('%H:%M:%S') + ' @ ' + tmp_bossTime[i].strftime('%Y-%m-%d') + ' (未入力 ' + str(bossMungCnt[i]) + '回)' + ' * ' + bossData[i][6] + '\n'
						else : 
							information1 += ' - ' + bossData[i][0] + '(' + bossData[i][1] + '.' + bossData[i][5] + ') : ' + tmp_bossTime[i].strftime('%H:%M:%S') + ' @ ' + tmp_bossTime[i].strftime('%Y-%m-%d') + ' (わきなし ' + str(bossMungCnt[i]) + '回)' + ' * ' + bossData[i][6] + '\n'
					else:
						if bossData[i][2] == '0' :
							information1 += ' - ' + bossData[i][0] + '(' + bossData[i][1] + '.' + bossData[i][5] + ') : ' + bossTimeString[i] + ' @ ' + bossDateString[i] + ' (未入力 ' + str(bossMungCnt[i]) + '回)' + ' * ' + bossData[i][6] + '\n'
						else : 
							information1 += ' - ' + bossData[i][0] + '(' + bossData[i][1] + '.' + bossData[i][5] + ') : ' + bossTimeString[i] + ' @ ' + bossDateString[i] + ' (わきなし ' + str(bossMungCnt[i]) + '回)' + ' * ' + bossData[i][6] + '\n'
	...
	if len(beforeBossData) > 1:	
		...
		for j in range(fixed_bossNum):
			try:
				...
				raise Exception(f"[fixed_boss.ini] ファイルの {tmp_fixed_bossData[j][0]} がエラー")
...
class taskCog(commands.Cog): 
	...
	async def command_task_list(self, ctx : commands.Context):
		...
		for t in asyncio.Task.all_tasks():
			...
			if t._coro.__name__ == f"boss_check":
				if t.done():
					...
					continue
				t.cancel()
		# await ctx.send( '< BOT再起動します。 >', tts=False)
		try:
		...
		if basicSetting[21] != "1":
			print("BOT再起動完了")
			await dbLoad()
			await self.bot.get_channel(channel).send( '< BOT再起動完了 >', tts=False)
	...
	async def boss_check(self):
				...
				if basicSetting[21] == "1":
					try:
						...
						if self.bot.voice_clients[0].is_connected() :
							await self.bot.get_channel(channel).send( '< BOT復帰 >', tts=False)
							self.checker = True
							print("BOT復帰")
					except:
						await self.bot.get_channel(channel).send( '< voiceチャンネル接続エラー >', tts=False)
						self.checker = False
						print("エラー")
						pass
					await dbLoad()
		...
		while True:
			...
			if log_stream.getvalue().find("Awaiting") != -1:
				...
				log_stream.seek(0)
				await self.bot.get_channel(channel).send( '< Discord接続エラー >', tts=False)
				await dbSave()
				break
						...
						if (bossTime[i]+datetime.timedelta(days=-365)) <= aftr:
							if basicSetting[2] != '0' and basicSetting[22] != '0' and bossFlag[i] == True and bossFlag0[i] == True and bossMungFlag[i] == True :
								if int(basicSetting[17]) <= bossMungCnt[i] and int(basicSetting[17]) != 0:
									...
									bossMungCnt[i] = 0
									if bossData[i][2] == '0':
										await self.bot.get_channel(channel).send(f'```未入力回数が {basicSetting[17]}回を超えたため [{bossData[i][0]}] 情報初期化します```', tts=False)
										print ('自動未入力処理 <' + bossData[i][0] + ' 初期化>')
									else:
										await self.bot.get_channel(channel).send(f'```わきなし回数が {basicSetting[17]}回を超えたため [{bossData[i][0]}] 情報初期化します```', tts=False)
										print ('自動わきなし処理 <' + bossData[i][0] + ' 初期化>')
									#await dbSave()
...
class mainCog(commands.Cog): 
	...
	async def join_(self, ctx):
		...
		if basicSetting[7] == "":
			...
			await ctx.send(f"< チャンネル [{ctx.message.channel.name}] 接続完了 >", tts=False)
	...
	async def menu_(self, ctx):
		if ctx.message.channel.id == basicSetting[7]:
			...
			command_list += ','.join(command[7]) + '初期化します\n'     #!초기화
			...
			command_list += ','.join(command[14]) + ' 0000, 00:00 全ボスタイムを一括セットします。\n'     #!보스일괄
			...
			command_list += ','.join(command[22]) + ' ボスタイム一括表示\n'     #보스탐
			command_list += ','.join(command[23]) + ' ボスタイム一括表示\n'     #!보스탐
			command_list += '[ボス名]END または [ボス名]エンド 0000, 00:00  倒した時間記録\n'  
			command_list += '[ボス名] END または [ボス名] エンド 0000, 00:00  倒した時間記録\n'   
			command_list += '[ボス名]わきなし または [ボス名] わきなし 0000, 00:00  わきなし記録\n'     
			command_list += '[ボス名]予想 または [ボス名] 予想 0000, 00:00  予想時間入力\n' 
	...
	async def initVal_(self, ctx):
		...
		if ctx.message.channel.id == basicSetting[7]:
			...
			await ctx.send('<初期化完了しました>', tts=False)
			print ("< 초기화 완료 >")
		else:
			return
	...
	async def allBossInput_(self, ctx):
		...
		if ctx.message.channel.id == basicSetting[7]:
			...
			await ctx.send('<ボスタイム一括入力完了>', tts=False)
			print ("<보스 일괄 입력 완료>")
		else:
			return
		...
		if chflg == 1 :
			if self.get_channel(basicSetting[7]).id == msg.channel.id:
				...
				for i in range(bossNum):
					...
					if message.content.startswith(bossData[i][0] +'わきなし') or message.content.startswith(bossData[i][0] +' わきなし') or message.content.startswith(bossData[i][0] +'SKIP') or message.content.startswith(bossData[i][0] +' SKIP'):
						if hello.find('  ') != -1 :
							bossData[i][6] = hello[hello.find('  ')+2:]
							hello = hello[:hello.find('  ')]
						else:
				...
				################ 예상 보스 타임 입력 ################ 
					...
					if message.content.startswith(bossData[i][0] +'予想')  or message.content.startswith(bossData[i][0] +' 予想'):
						...
						if len(hello) > len(tmp_msg) + 4 :
							...
							embed = discord.Embed(
									description= '```'+ bossData[i][0] + 'の予想時間は' + bossTimeString[i] + 'です。```',
									color=0xff0000
									)
							await self.get_channel(channel).send(embed=embed, tts=False)
						else:
							await self.get_channel(channel).send('```' + bossData[i][0] +'の予想時間を入力してください。```', tts=False)
					...
					if message.content == bossData[i][0] +'DEL' or message.content == bossData[i][0] +' DEL':
						...
						bossMungFlag[i] = False
						bossMungCnt[i] = 0
						await self.get_channel(channel).send('<' + bossData[i][0] + 'の設定時間を消去しました>', tts=False)
						await dbSave()
						print ('<' + bossData[i][0] + 'の設定時間を消去しました>')
